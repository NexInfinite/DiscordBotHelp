...
local keyboardIdentifier = '772BDB1'
...
lmc_set_handler('MACROS', function(button, direction)
	--Ignoring upstrokes ensures keystrokes are not registered twice, but activates faster than ignoring downstrokes. It also allows press and hold behaviour
        ...
        if (button == 88) then
                lmc_send_keys('embed = discord.Embed{(}description=f"", ~color=discord.Color.dark_blue{(}{)}{)}~embed.set_author{(}name=f""{)}~await ctx.send{(}embed=embed{)}')
                end
        if (button == 67) then
                lmc_send_keys('embed.add_field{(}name=f"", value=f""{)}')
                end
        if (button == 39) then
                lmc_send_keys('{right 15}')
                end
        if (button == 37) then
                lmc_send_keys('{left 15}')
                end
